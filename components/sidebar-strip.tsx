...
export function SidebarStrip({ onSelectVideo, onSelectError, onBuyCredits }: SidebarStripProps) {
  ...
  const videoThumbnail = (vid: ShowcaseVideo, i: number) => (
    <button
      key={i}
      className="group relative h-10 w-10 shrink-0 overflow-hidden rounded-lg bg-neutral-50 ring-1 ring-neutral-200 md:h-12 md:w-12"
  ...
  return (
    <>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-1.5">
          ...
          {showcaseVideos.length > 0 && (
            <div className="mt-2 flex flex-col items-center gap-2">
              ...
              {showcaseVideos.map((vid, i) => videoThumbnail(vid, i))}
            </div>
          )}
        </div>
      ...
      <div className="fixed inset-x-0 bottom-0 z-30 flex items-center gap-2 border-t border-neutral-200 bg-white/95 px-3 pb-[max(0.5rem,env(safe-area-inset-bottom,0.5rem))] pt-2 backdrop-blur-sm md:hidden">
        {/* User */}
        {user ? (
          <button
            onClick={() => setShowMenu(!showMenu)}
